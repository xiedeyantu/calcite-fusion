...
public enum EnumerableConvention implements Convention {
  ...
  @Override public RelNode enforce(
    ...
    RelCollation requiredCollation = required.getCollation();
    RelCollation collation = rel.getTraitSet().getTrait(RelCollationTraitDef.INSTANCE);
    boolean collationChanged = false;
    if (collation != null && !collation.satisfies(requiredCollation)) {
      collationChanged = true;
    }

    RelDistribution requiredDistribution = required.getDistribution();
    RelDistribution distribution = rel.getTraitSet().getTrait(RelDistributionTraitDef.INSTANCE);
    boolean distributionChanged = false;
    if (distribution != null && !distribution.satisfies(requiredDistribution)) {
      distributionChanged = true;
    }

    if (distributionChanged) {
      return EnumerableSortExchange.create(rel,
          requiredDistribution,
          collationChanged ? requiredCollation : RelCollations.of(),
          rel.getConvention());
    } else if (collationChanged) {
      return EnumerableSort.create(rel, requiredCollation, null, null);
    } else {
      return rel;
    }
  }
